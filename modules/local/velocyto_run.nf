process VELOCYTO_RUN {
    ...
    input:
    tuple val(key), path(cellranger_dir), path(barcodes_tsv), path(gtf), val(sample_id), val(publish_dir)

    output:
    tuple val(key), path("${sample_id}/possorted_genome_bam.loom"), emit: loom
